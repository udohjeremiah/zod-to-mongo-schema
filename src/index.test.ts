...
describe("zod-to-mongo-schema", () => {
  describe("strict mode (default)", () => {
    ...
    it("converts `float32` and `float64` ranges to `double`, and custom ones to `number`", () => {
      ...
      expect(r.properties?.float32DefaultRange.minimum).toBeCloseTo(
        FLOAT32_MIN,
      );
      expect(r.properties?.float32DefaultRange.maximum).toBeCloseTo(
        FLOAT32_MAX,
      );
    ...
    it("doesn't allow both `type` and `bsonType` simultaneously", () => {
      const badSchemas = [
        ...
        z.object({
          field: z.unknown().meta({ type: "null", bsonType: "date" }),
        }),
      ];
    ...
    it("handles nested schema arrays (`allOf`/`anyOf`/`items`) and strips unknown keys", () => {
      ...
      expect(r.properties?.exactly?.allOf?.[1]).toMatchObject({
        type: "number",
      });

      expect(r.properties?.either.anyOf).toHaveLength(2);
      expect(r.properties?.either?.anyOf?.[0]).toMatchObject({
        type: "string",
      });
    ...
    it("auto-maps z.date() to { bsonType: 'date' }", () => {
      const schema = z.object({
        createdAt: z.date(),
      });
    ...
    it("auto-maps z.date() inside containers (optional, nullable, array)", () => {
      const schema = z.object({
        a: z.date().optional(),
        b: z.date().nullable(),
        c: z.array(z.date()),
      ...
      expect(r.properties?.b).toMatchObject({
        anyOf: [{ bsonType: "date" }, { type: "null" }],
      });
      expect(r.properties?.c).toMatchObject({
        type: "array",
        items: { bsonType: "date" },
      });
    });

    it("throws on z.date().meta({ bsonType }) in strict mode", () => {
      const schema = z.object({
    ...
    it("throws on unrepresentable types like z.symbol()", () => {
      const schema = z.object({
        sym: z.symbol(),
      });
    ...
    it("does not falsely detect z.any() as unrepresentable", () => {
      const schema = z.object({
        val: z.any().optional(),
      });
  ...
  describe("non-strict mode (strict: false)", () => {
    it("allows `bsonType` on non-`z.unknown()` types", () => {
      const schema = z.object({
        age: z.number().meta({ bsonType: "int" }),
        name: z.string().meta({ bsonType: "string" }),
      ...
      expect(r.properties?.isActive).toMatchObject({ bsonType: "bool" });
    });

    it("handles z.date() with bsonType override", () => {
      const schema = z.object({
        createdAt: z.date().meta({ bsonType: "date" }),
    ...
    it("handles z.instanceof(Uint8Array) with bsonType override", () => {
      const schema = z.object({
    ...
    it("allows `bsonType` to override `type` when both are specified", () => {
      const schema = z.object({
        field: z.unknown().meta({ type: "null", bsonType: "bool" }),
      });
      ...
      expect((r.properties?.field as any).type).toBeUndefined();
    });

    it("converts unrepresentable types to empty schema without bsonType", () => {
      const schema = z.object({
    ...
    it("maintains existing behavior for representable types", () => {
      const schema = z.object({
        name: z.string(),
        age: z.number(),
        isStudent: z.boolean(),
      ...
      expect(r.properties?.isStudent).toMatchObject({ type: "boolean" });
    });
  });
});
